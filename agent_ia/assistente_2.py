...
def cadastrar_transacao(valor: float, tipo: str, descricao: str = None, categoria: str = None, account_id: str = None, state: dict = None, thread_id: str = None) -> str:
    ...
    Args:
        ...
        account_id: ID (uuid) da conta ou nome da conta (ex: "Santander CC", "C6") — opcional
    ...
    try:
        ...
        # PARTE 3 — Detecção automática de cartão
        cartoes = []
        contas_normais = []
        for conta in contas_ativas:
            nome = conta.get("nome", "").lower()
            if "cc" in nome or "cartao" in nome or "crédito" in nome or "credito" in nome:
                cartoes.append(conta)
            else:
                contas_normais.append(conta)

        account_id_param = account_id  # guardar parâmetro para PARTE 4
        descricao_conta = (descricao or "").lower()
        if "cartao" in descricao_conta or "credito" in descricao_conta or "crédito" in descricao_conta:
            if len(cartoes) == 1:
                account_id = cartoes[0]["id"]

        # PARTE 4 — Conversão nome da conta → ID (se agent enviou account_id como nome)
        conta_id_final = None
        if account_id_param:
            for conta in contas_ativas:
                if str(conta.get("id")) == str(account_id_param):
                    conta_id_final = conta["id"]
                    break
                nome_conta = conta.get("nome", "").lower()
                if str(account_id_param).lower() in nome_conta:
                    conta_id_final = conta["id"]
                    break
            if conta_id_final:
        ...
        if not account_id or account_id not in ids_validos:
            if len(contas_ativas) == 1:
                account_id = contas_ativas[0]["id"]
            else:
                # PARTE 6 — Proteção contra loop
                if state and state.get("aguardando_conta"):
                    return "Ainda preciso saber qual conta foi utilizada."
        ...
        # PARTE 1 — Lista de categorias do usuário
        categorias_usuario = user_doc.get("categorias", {})
        todas_categorias = []
        for lista in categorias_usuario.values():
            todas_categorias.extend(lista)

        # Classificação automática de categoria (keywords + match direto + fallback)
        descricao_norm = (descricao or "").lower()
        categoria_final = None

        # ------------------------------------------------
        # MAPA DE PALAVRAS-CHAVE COMUNS
        # ------------------------------------------------
        mapa_keywords = {
            "uber": "Táxi/Uber",
            "99": "Táxi/Uber",
            ...
            "restaurante": "Refeições fora de casa",
            "almoco": "Refeições fora de casa",
            "jantar": "Jantares",
            "delivery": "Delivery"
        }

        # ------------------------------------------------
        # 1️⃣ TENTAR MAPA DE KEYWORDS
        # ------------------------------------------------
        for palavra, cat in mapa_keywords.items():
            if palavra in descricao_norm:
                for categoria_usuario in todas_categorias:
                    if categoria_usuario.lower() == cat.lower():
                        categoria_final = categoria_usuario
                        break
                if categoria_final:
                    break

        # ------------------------------------------------
        # 2️⃣ MATCH DIRETO POR TEXTO
        # ------------------------------------------------
        if not categoria_final:
            for cat in todas_categorias:
                cat_norm = cat.lower()
                if cat_norm in descricao_norm or descricao_norm in cat_norm:
                    categoria_final = cat
                    break
